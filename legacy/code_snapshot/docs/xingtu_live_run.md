...
```bash
cd /home/tuo/code/apps/api
/home/tuo/code/.venv/bin/python run_xingtu_flow.py \
  --storage-state /home/tuo/playwright/.auth/xingtu.json \
...
```bash
cd /home/tuo/code/apps/api
export XINGTU_STORAGE_STATE=/home/tuo/playwright/.auth/xingtu.json
...
```json
{
  "storage_state_path": "/home/tuo/playwright/.auth/xingtu.json",
  "account_name": "保乐力加-因赛ID：1815512192324739",
  "headless": false,
  "row_limit": 3,
  "target_row_index": 0,
  "filter_config": {
...
```bash
cd /home/tuo/code/apps/api
/home/tuo/code/.venv/bin/python run_xingtu_flow.py --config /path/to/xingtu_live_config.json
```

## 运行前准备

当前建议直接使用仓库根目录下的项目虚拟环境：

```bash
cd /home/tuo/code
/home/tuo/code/.venv/bin/python -m pip install -r apps/api/requirements.txt playwright
/home/tuo/code/.venv/bin/python -m playwright install chromium
```
...
/home/tuo/code/.venv/bin/python run_xingtu_flow.py \
  --storage-state /home/tuo/playwright/.auth/xingtu.json \
  --account-name '保乐力加-因赛ID：1815512192324739' \
  --filters-json '{"named_filters":{"美妆":["美妆测评种草"]}}' \
  --row-limit 3 \
  --row-index 0 \
  --headed
...
```bash
cd /home/tuo/code
/home/tuo/code/.venv/bin/python -m unittest discover -s apps/api/tests -v
```
